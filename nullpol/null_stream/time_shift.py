...
def time_shift(interferometers, ra, dec, gps_time, strain_data_array):
    """Time shift.
    ...
    """
    time_shift = np.conj(np.array([np.exp(-1.j*np.pi*2.*interferometer.frequency_array*interferometer.time_delay_from_geocenter(ra, dec, gps_time)) for interferometer in interferometers]))
