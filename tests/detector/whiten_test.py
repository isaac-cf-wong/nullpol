...
class TestWhiten(unittest.TestCase):
    ...
    def test_compute_whitened_frequency_domain_strain_array(self):
        ...
        """
        frequency_domain_strain_array = np.array([ifo.frequency_domain_strain for ifo in self.ifos])
        power_spectral_density_array = np.array([ifo.power_spectral_density_array for ifo in self.ifos])
        whitened_frequency_domain_strain_array = compute_whitened_frequency_domain_strain_array(
            frequency_mask=self.frequency_mask,
            frequency_resolution=1.0 / self.duration,
