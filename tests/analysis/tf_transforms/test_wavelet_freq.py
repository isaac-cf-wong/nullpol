...
class TestWaveletFreqHelpers:
    ...
    def test_wavelet_freq_quadrature_consistency_with_high_level(self):
        ...
        Nt, Nf = high_level_result.shape
        phif = 2 / Nf * _phitilde_vec_norm(Nf, Nt, nx)
        low_level_result = _transform_wavelet_freq_quadrature_helper(data_fft, Nf, Nt, phif) * np.sqrt(time_domain_length)
        
        # Should produce very similar results
        assert np.allclose(high_level_result, low_level_result, rtol=1e-10), "Quadrature high and low level should match"
    ...
    def test_wavelet_freq_helpers_energy_conservation(self):
        ...
        assert np.isclose(input_energy, output_energy, rtol=0.1), f"Energy not conserved: {input_energy} vs {output_energy}"
        assert np.isclose(input_energy, output_energy_quad, rtol=0.1), f"Quadrature energy not conserved: {input_energy} vs {output_energy_quad}"
