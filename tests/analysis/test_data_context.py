...
def test_compute_time_shifted_frequency_domain_strain_basic():
    ...
    assert np.isclose(shifted_strain[test_freq_bin], expected_value, rtol=1e-10), \
        f"Expected {expected_value}, got {shifted_strain[test_freq_bin]}"
    
    # Test that amplitude is preserved (time shift only changes phase)
    assert np.isclose(abs(shifted_strain[test_freq_bin]), 
                     abs(frequency_domain_strain[test_freq_bin]), rtol=1e-10), \
...
def test_compute_time_shifted_frequency_domain_strain_array():
    ...
    for i, time_delay in enumerate(time_delays):
        ...
        assert np.isclose(shifted_array[i, test_freq_bin], expected_value, rtol=1e-10), \
...
class TestTimeFrequencyDataContext:
    ...
    def test_initialization_basic(self, detector_setup):
        ...
        assert context.sampling_frequency == sampling_frequency, f"Expected sampling_frequency {sampling_frequency}, got {context.sampling_frequency}"
        assert len(context.interferometers) == len(ifos), f"Expected {len(ifos)} interferometers, got {len(context.interferometers)}"
    ...
    def test_compute_time_delay_array(self, detector_setup):
        ...
        individual_delay = ifos[0].time_delay_from_geocenter(
            ra=parameters_overhead['ra'], 
            dec=parameters_overhead['dec'], 
            time=parameters_overhead['geocent_time']
        )
