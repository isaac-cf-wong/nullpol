...
def test_get_antenna_pattern_known_values():
    ...
    antenna_response = get_antenna_pattern(
        h1, ra, dec, psi, gps_time, polarization_mask
    )
    ...
    antenna_response_rotated = get_antenna_pattern(
        h1, ra, dec, psi_90, gps_time, polarization_mask
    )
    ...
    assert not np.allclose(antenna_response, antenna_response_rotated), \
        "Response should change with polarization angle"
...
def test_get_antenna_pattern_matrix_symmetry():
    ...
    antenna_matrix = get_antenna_pattern_matrix(
        ifos, ra, dec, psi, gps_time, polarization_mask
    )
    ...
    assert not np.allclose(h1_response, l1_response, atol=1e-10), \
        "H1 and L1 should have different antenna responses"
    
    # Test consistency: same detector should give same result
    single_h1_response = get_antenna_pattern(
        ifos[0], ra, dec, psi, gps_time, polarization_mask
    )
    assert np.allclose(antenna_matrix[0, :], single_h1_response), \
        "Matrix and individual computations should match"
...
def test_relative_amplification_factor_simple_case():
    ...
    assert np.array_equal(param_map, expected_map), \
        f"Expected {expected_map}, got {param_map}"
    ...
    assert np.iscomplex(amplification_factors[0, 1]) or np.isreal(amplification_factors[0, 1]), "bc factor should be complex"
...
def test_get_collapsed_antenna_pattern_matrix_simple_example():
    ...
    assert np.allclose(collapsed_matrix, expected_result), \
        f"Expected\n{expected_result}\nGot\n{collapsed_matrix}"
