...
class TestAntennaPatternProcessor:
    ...
    def test_detector_count_validation_pass(self):
        ...
        try:
            ...
            assert len(ifos) > np.sum(processor.polarization_basis), \
                "Validation should ensure detectors > basis modes"
        except NullpolError:
    ...
    def test_compute_antenna_pattern_matrix_basic(self, sample_interferometers):
        ...
        antenna_matrix = processor.compute_antenna_pattern_matrix(
            sample_interferometers, parameters
        )
        ...
        assert antenna_matrix.shape == expected_shape, \
            f"Expected shape {expected_shape}, got {antenna_matrix.shape}"
    ...
    def test_relative_amplification_factor_map_property(self, sample_interferometers):
        ...
        assert actual_shape == expected_shape, \
            f"Amplification map shape: expected {expected_shape}, got {actual_shape}"
        ...
        assert actual_entries == expected_entries, \
            f"Expected map entries {expected_entries}, got {actual_entries}"
    ...
    def test_compute_whitened_antenna_pattern_matrix_method(self, sample_interferometers):
        ...
        assert whitened_matrix.shape == expected_shape, \
            f"Expected shape {expected_shape}, got {whitened_matrix.shape}"

        # Check that masked frequencies are zero
        assert np.all(whitened_matrix[50:, :, :] == 0), \
            "Masked frequencies should have zero values"

        # Check that active frequencies have non-zero values
        assert np.any(whitened_matrix[:50, :, :] != 0), \
            "Active frequencies should have non-zero values"

        # All active values should be finite
        assert np.all(np.isfinite(whitened_matrix[:50, :, :])), \
            "All active whitened values should be finite"
    ...
    def test_compute_calibration_factor_matrix_method(self, sample_interferometers):
        ...
        frequency_domain_strain_array = np.random.normal(0, 1e-23, (n_detectors, n_frequencies)) + \
        ...
        try:
            calibration_matrix = processor.compute_calibration_factor_matrix(
                sample_interferometers, frequency_domain_strain_array, 
                masked_frequency_array, frequency_mask, parameters
            )
            ...
            # All values should be finite
            assert np.all(np.isfinite(calibration_matrix)), \
                "All calibration factors should be finite"
    ...
    def test_compute_calibrated_whitened_antenna_pattern_matrix_method(self, sample_interferometers):
        ...
        try:
            calibrated_whitened_matrix = processor.compute_calibrated_whitened_antenna_pattern_matrix(
                sample_interferometers, power_spectral_density_array, 
                frequency_domain_strain_array, masked_frequency_array, 
                frequency_mask, parameters
            )
            ...
            # Check that masked frequencies are zero  
            assert np.all(calibrated_whitened_matrix[25:, :, :] == 0), \
                "Masked frequencies should have zero values"

            # Check that active frequencies have values (may be complex)
            assert np.any(calibrated_whitened_matrix[:25, :, :] != 0), \
                "Active frequencies should have non-zero values"
