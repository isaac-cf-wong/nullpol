...
import scipy.stats
import scipy.signal
from nullpol.time_frequency_transform import get_shape_of_stft
from nullpol.injection import create_injection
from nullpol.detector import (simulate_stft_psd,
...
class TestInterferometer(unittest.TestCase):
    def setUp(self):
        ...
        self.start_time = 0
        self.frequency_resolution = 16
        self.nx = 4.
        self.nsample = 1000
        self.window_function = scipy.signal.windows.tukey(int(self.sampling_frequency/self.frequency_resolution), 0.1)
        self.window_function /= np.sqrt(np.mean(self.window_function**2))
    ...
    def test_whiten_noise(self):
        ...
        create_injection(interferometers=interferometers,
                         ...
                         noise_type='noise')
        # Create the time_frequency_filter
        Nt, Nf = get_shape_of_stft(duration=interferometers[0].duration,
                                   sampling_frequency=interferometers[0].sampling_frequency,
                                   frequency_resolution=self.frequency_resolution)
        time_frequency_filter = np.full((Nt, Nf), True).astype(bool)
        min_k = int(np.ceil(self.minimum_frequency / self.frequency_resolution))
        max_k = int(np.floor(self.maximum_frequency / self.frequency_resolution))
        time_frequency_filter[:, :min_k] = False
        time_frequency_filter[:, max_k:] = False
        # Simulate the wavelet PSD
        time_frequency_domain_strain_array = []
        psd_array = []
        for interferometer in interferometers:
            stft_psd = simulate_stft_psd(interferometer=interferometer,
                                         frequency_resolution=self.frequency_resolution,
