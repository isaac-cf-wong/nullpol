...
This directory contains tooling to automatically generate `__init__.py` files
for leaf packages in the nullpol codebase.

## Overview

The nullpol package uses a hybrid approach for `__init__.py` management:

- **High-level packages**: Manually maintained (e.g., `src/nullpol/`,
  `src/nullpol/analysis/`, `src/nullpol/integrations/`)
- **Leaf packages**: Auto-generated (e.g., `src/nullpol/simulation/`,
  `src/nullpol/utils/`, `src/nullpol/analysis/likelihood/`)

This eliminates star imports (`from .module import *`) in favor of explicit
imports while reducing developer maintenance burden.
...
2. Identifies "leaf packages" (directories containing only `.py` files, no
   subdirectories)
...
The script runs automatically as a pre-commit hook when Python files in `src/`
are modified:
...
The script also runs in GitLab CI to ensure auto-generated files stay up to
date:

- **`init-files-check` job**: Validates that all auto-generated `__init__.py`
  files are current
- Runs before code style checks (`black`, `ruff`) in the `code-quality` stage
- Uses `--quiet` mode for clean CI output
- Fails the pipeline if files are out of sync, with clear instructions to run
  the script locally
...
High-level packages are excluded from auto-generation and should be manually
maintained:
...
**Template directories** are automatically excluded since they contain no `.py`
files.
...
- `data/`, `resources/` - Data/resource files (when not containing Python
  modules)
...
- ✅ `src/nullpol/simulation/` → Leaf (only `.py` files: injection.py,
  source.py)
- ✅ `src/nullpol/utils/` → Leaf (only `.py` files: config.py, error.py, etc.)
- ✅ `src/nullpol/analysis/prior/` → Leaf (has `.py` files + `prior_files/`
  non-code subdir)
- ❌ `src/nullpol/analysis/` → Not leaf (has code subdirs like `likelihood/`,
  `clustering/`)
...
- `--dry-run`: Always shows what would change (combines well with `--verbose` or
  `--quiet`)
...
4. **Reduced maintenance**: Developers don't need to manually update leaf
   `__init__.py` files
...
For high-level packages, manually edit the `__init__.py` to control the public
API.
