...
def _phitilde_vec(om: np.ndarray, Nf: int, nx: float = 4.0) -> np.ndarray:
...
def _phitilde_vec_norm(Nf: int, Nt: int, nx: float) -> np.ndarray:
...
@njit
def _tukey(data: np.ndarray, alpha: float, N: int) -> None:
...
def _transform_wavelet_freq_helper(data: np.ndarray, Nf: int, Nt: int, phif: np.ndarray) -> np.ndarray:
    ...
    for m in range(0, Nf + 1):
        ...
        _DX_unpack_loop(m, Nt, Nf, DX_trans, wave)
    return wave


def _transform_wavelet_freq_quadrature_helper(data: np.ndarray, Nf: int, Nt: int, phif: np.ndarray) -> np.ndarray:
...
@njit
def _DX_assign_loop(m: int, Nt: int, Nf: int, DX: np.ndarray, data: np.ndarray, phif: np.ndarray) -> None:
...
@njit
def _DX_unpack_loop(m: int, Nt: int, Nf: int, DX_trans: np.ndarray, wave: np.ndarray) -> None:
...
@njit
def _DX_unpack_loop_quadrature(m: int, Nt: int, Nf: int, DX_trans: np.ndarray, wave: np.ndarray) -> None:
    """Helper for unpacking fftd DX in main loop.
