...
def bilby_config_to_asimov(config_name):
    ...
    output_dict["psds"] = _convert_string_to_dict(config["psd-dict"])
    ...
    fields = {
        "spline-calibration-envelope-dict": "calibration",
    ...
    fields = {"time-reference": "time reference", "reference-frame": "reference frame"}
    for name_ini, name_asimov in fields.items():
        if name_ini in config:
    ...
    fields = {
        "post-trigger-duration": "post trigger time",
    ...
    marginalization = {}
    fields = {
        "distance-marginalization": "distance",
    ...
    if config["prior-dict"] != "None":
        prior_dict = bilby_pipe.utils.convert_prior_string_input(config["prior-dict"])
        prior_dict = {key.replace("-", "_"): value for key, value in prior_dict.items()}
            ...
            if name_ini in prior_dict:
                ...
                for parameter in prior_rest:
                    ...
                    if key not in ["name", "unit", "latex_label"]:
                        if key == "boundary":
                            param_prior[key] = _convert_string_to_dict(value)
                        else:
                            try:
    ...
    fields = {"default-prior": "default", "calibration-prior-boundary": "boundary"}
    for name_ini, name_asimov in fields.items():
        if name_ini in config:
            value = config[name_ini]
            if value is not None:
